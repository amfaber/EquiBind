...
class Ligands(Dataset):
    def __init__(self, ligs, args, n_jobs = 8, verbose = 1):
        self.n_jobs = n_jobs
        ...
        to_remove = {i for i in range(len(lig_graphs)) if lig_graphs[i] is None}
        [print(f"Graph generation failed for {ligs[i].GetProp('_Name')}") for i in to_remove]

        def remove_indices(List, indices):
            return [List[i] for i in range(len(List)) if i not in indices]
        
        self.ligs = remove_indices(ligs, to_remove)
        self.lig_graphs = remove_indices(lig_graphs, to_remove)
        self.geometry_graphs = remove_indices(geometry_graphs, to_remove)
    ...
    def __getitem__(self, idx):
        return self.ligs[idx], self.lig_graphs[idx], self.geometry_graphs[idx]

    def collate(self, _batch):
        ligs, lig_graphs, geometry_graphs = map(list, zip(*_batch))
        return ligs, batch(lig_graphs), batch(geometry_graphs)
